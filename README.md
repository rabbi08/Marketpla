...
Browser support is defined in the `marketplace/browserslist` file that is used for autoprefixing CSS.
...
|run psql                              |`make psql`                            |`docker-compose exec postgres psql --user marketplace --dbname marketplace` |
|generate docs                         |`make docs`                            |`docker-compose run --rm django sphinx-build ./docs ./docs/_build`          |

## Running commands on the server

    docker-compose -f docker-compose.production.yml run --rm --name marketplace_tmp django python manage.py <command>
...
- python: [`marketplace/cover`](./marketplace/cover)
