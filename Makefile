...
.PHONY:
black-check:
	@make run-python cmd="black --check $(cmd)"


.PHONY:
black-check-all:
	@make run-python cmd="black --check ."
...
.PHONY:
settings: poetry.lock $(SITE_ROOT)/settings/local.py $(SITE_ROOT)/settings/local_test.py
...
makemessages:
	...
	docker-compose run --rm django ./manage.py makemessages -a -d djangojs --ignore="docs/*"
